...
원하는 만큼 데이터를 사용하여 일괄 시트를 생성하거나 선택적으로 수동으로 생성하여 업로드할 수 있습니다. 파일 형식 요구 사항에 대해서는 &quot;[Bulksheets에 필수 및 포함 데이터](/help/search-social-commerce/campaign-management/bulksheets/bulksheet-data-formats/bulksheet-file-formats.md)&quot;을, 광고 네트워크에서 지원되는 작업에 대해서는 &quot;[Bulksheets에서 수행할 수 있는 작업](/help/search-social-commerce/campaign-management/bulksheets/bulksheet-data-formats/bulksheet-operations.md)&quot;을(를) 참조하십시오.
...
## 광고 네트워크별 기능 {#bulksheet-functionality-by-network}
...
동기화된 계정이 있는 일괄 시트를 사용하는 표준 단계는 다음과 같습니다.

1. [하나 이상의 계정, 캠페인 또는 광고 그룹에 대한 데이터를 일괄 시트 파일로 다운로드](download.md). 선택적으로 광고 네트워크별 일괄 시트를 수동으로 채우고 파일을 업로드할 수 있습니다.

1. 파일의 기본(최종) URL 또는 대상 URL에서 [랜딩 페이지의 유효성 검사](validate-landing-pages.md).

1. 데이터를 추가하거나 수정해야 하는 경우:

   1. 파일을 데스크톱으로 내보내고 [!DNL Microsoft Excel]에서 편집하세요.

   1. [편집된 파일을 수동으로 업로드](upload.md)하여 검색, 소셜 및 Commerce에 업로드합니다.

1. (수동으로 업로드한 파일의 경우) [파일을 업로드할 때 또는 나중에 광고 네트워크에 게시](post.md)합니다.

1. (필요한 경우) 새 오류 파일을 다운로드하고 행을 수정한 다음 파일을 다시 게시합니다.

## 캠페인 데이터 업로드 및 게시 오류 관리

검색, 소셜 및 Commerce은 필요할 때 생성하는 추적 URL을 포함하여 캠페인 일괄 시트에서 가능한 한 많은 데이터 행을 업로드하고 게시합니다.
...
다운로드한 모든 데이터 파일, 업로드한 파일 및 오류 파일은 [!UICONTROL Setup] \> [!UICONTROL Bulksheets]에서 사용할 수 있습니다.
