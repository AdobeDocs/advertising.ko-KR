...
* *[!UICONTROL Content]:* 콘텐츠(디스플레이) 네트워크 목록에 배치 입찰을 진행합니다. **참고:** 템플릿을 사용하여 배치를 만들 수 없습니다. 이 옵션을 선택하는 경우 각 광고 그룹에 대한 배치를 만들고 일괄 시트 또는 [!UICONTROL Search] > [!UICONTROL Campaigns] 보기의 광고 그룹 및 배치 설정을 사용하여 각 광고 그룹에 대해 타깃팅할 표시 네트워크의 페이지를 지정합니다.
