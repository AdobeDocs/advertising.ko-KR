...
* **오류:** 일괄 시트 작업에서 파일이나 파일의 개별 행을 업로드하거나 처리할 수 없으면 이라는 오류 파일이 만들어집니다 `<uploaded file name>_ef_errors.<extension used for the bulksheet>`. 개별 행에 문제가 있는 경우 해당 행이 각 오류에 대한 설명과 함께 포함되므로 수정할 수 있습니다. 오류는 &quot;에 포함됩니다.[!UICONTROL EF Error Message]&quot;열.

## [!UICONTROL SE Error] 메시지

의 오류 [!UICONTROL SE Error] 열은 광고 네트워크에서 직접 가져옵니다.

## [!UICONTROL EF Error] 메시지

에는 다음 오류가 포함될 수 있습니다. [!UICONTROL EF Error] 열 위치 [!UICONTROL EF Errors] 파일.
...
|  | [!UICONTROL No corresponding keyword for Adgroup] | ([!DNL Yandex]) 광고 그룹에 하나 이상의 키워드가 포함되어 있지 않아서 광고 그룹이 만들어지지 않았습니다. |
|  | [!UICONTROL No corresponding creative for Adgroup] | ([!DNL Yandex]) 광고 그룹에 광고가 하나 이상 포함되어 있지 않아서 광고 그룹이 만들어지지 않았습니다. |
|  | [!UICONTROL Creative Row Missing] | ([!DNL Yandex]) 광고 그룹에 광고가 하나 이상 포함되어 있지 않아서 광고 그룹이 만들어지지 않았습니다. |
...
|  | [!UICONTROL Cannot modify Tracking Template at Keyword / Creative / Site Link level until Account has been migrated to use Upgraded URLs. Please retry after migration] | &quot;[!UICONTROL Tracking Template]&quot;필드는 최종/고급 URL을 사용하는 계정에만 해당됩니다. 최종/고급 URL을 사용하도록 계정을 마이그레이션하기 전까지 값을 제거합니다. |
...
| 쇼핑 제품 그룹 | [!UICONTROL Cannot delete Everything Else manually. It will be deleted automatically when all Product Group Conditions under the same parent are removed] | 제품 그룹의 각 수준에는 &quot;[!UICONTROL Everything Else]&quot; 그룹입니다. 은(는) 수동으로 삭제할 수 없습니다.[!UICONTROL Everything Else]&quot;그룹. 동일한 수준에서 다른 모든 제품 그룹을 삭제하면 자동으로 삭제됩니다. |
...
| Sitelink | [!UICONTROL Sitelinks Cannot update more than 10 site links per campaign] | ([!DNL Yandex] 전용) 메시지가 부정확합니다. 각 캠페인에는 최대 4개(10개가 아님) 사이트 링크가 포함될 수 있으며 이 일괄 시트에는 4개 이상이 포함됩니다. 일부 사이트링크를 제거합니다. |
|  | [!UICONTROL Cannot update sitelinks under deleted/expired campaign] | 상위 캠페인이 만료되거나 삭제되었으므로 사이트 링크를 편집할 수 없습니다. |
|  | [!UICONTROL Creative creation failed] | ([!DNL Yandex]) 광고가 만들어지지 않았기 때문에 사이트 링크를 만들 수 없습니다. |
...
| 제외 | [!UICONTROL Other than status is modified] | 제외의 상태(&quot;만 편집할 수 있습니다.[!UICONTROL Active]&quot; 또는 &quot;[!UICONTROL Deleted]&quot;). |
| Google 리마케팅 목록 타겟/대상 | [!UICONTROL Invalid Audience given] | ([!DNL Google Ads] 캠페인 전용) RLSA 대상이 기존 RLSA(대상자)에 해당하지 않습니다. 의 값을 수정합니다.[!UICONTROL Audience]&quot; 및 &quot;[!UICONTROL RLSA Target]&quot;열. |

### 게시물 오류

에서 다음 오류가 발생합니다. [!UICONTROL EF Errors] 파일만. 대부분의 게시 오류는 광고 네트워크에서 발생하며 SE 오류 파일에 포함됩니다.
