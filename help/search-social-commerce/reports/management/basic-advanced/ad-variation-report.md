...
다음 [!UICONTROL Ad Variation Report] 하나 이상의 광고 그룹에서 노출 횟수를 수신한 각 적용 가능한 광고 변형(광고 크리에이티브)에 대한 비용, 클릭 및 (선택적으로) 전환 데이터를 포함합니다. 기본적으로 데이터에는 지정된 날짜 범위의 각 시간 단위에 대해 노출 횟수를 수신한 적용 가능한 각 광고에 대한 한 개의 행이 포함됩니다. 행은 먼저 광고 제목별로 오름차순으로 정렬한 다음 기본적으로 광고 네트워크별로 정렬됩니다.
...
>[!NOTE]
>
>다음에 대한 광고 수준 데이터를 사용할 수 없음: [!DNL Google Ads] 동적 검색 광고(DSA), 성능 최대, 스마트 쇼핑 및 [!DNL YouTube] 캠페인. 이러한 캠페인 유형이 있는 경우 캠페인에 대한 총 광고 수준 데이터와 캠페인에 대한 총 데이터 간에 불일치가 예상됩니다.
