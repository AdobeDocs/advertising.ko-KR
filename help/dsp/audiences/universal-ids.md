...
* 일부 공급업체의 서드파티 세그먼트에는 쿠키 또는 디바이스 ID로 추적되는 사용자 외에도 범용 ID가 자동으로 포함될 수 있습니다. 예를 들어 [!DNL Eyeota]의 세그먼트에는 ID5 ID가 자동으로 포함될 수 있고, [!DNL Lotame]의 세그먼트에는 UID2.0 ID가 포함될 수 있습니다. 세그먼트 세부 정보에는 각 유형의 크기가 포함됩니다. 세그먼트 이름 옆에 표시되는 각 세그먼트에 대한 일반적인 사용 요금이 적용됩니다. ID5 ID에 대해서는 추가 요금이 부과되지 않습니다.
