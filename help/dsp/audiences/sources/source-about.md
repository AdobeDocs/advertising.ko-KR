...
* 오스트레일리아의 광고주는 이미 [!DNL AdFixus]개의 범용 ID를 포함하는 자사 세그먼트를 가져올 수 있습니다.

* DSP은 추가 고객 데이터 플랫폼(CDP) 내에 구축된 해시된 이메일 ID, 쿠키 및 모바일 광고 ID(MAID)로 구성된 자사 세그먼트를 수집하여 [!DNL LiveRamp] [!DNL RampIDs] 및 [!DNL Unified ID 2.0 (UID2.0)] ID로 구성된 세그먼트로 변환할 수 있습니다.

모든 ID 유형의 경우 각 결과 ID는 사용자를 기반으로 하며 광고 빈도 상한은 ID 수준 <!-- Move that info. to somewhere else? -->에서 적용됩니다. 세그먼트 세부 정보에는 각 범용 ID 유형의 크기와 쿠키 또는 장치 ID로 추적되는 각 장치 유형의 크기가 포함됩니다.

## 자사 세그먼트를 번역할 수 있는 범용 ID 유형 {#universal-id-types}
...
[!DNL ActionIQ], [!DNL Adobe] [!DNL Real-time CDP], [!DNL Amperity], [!DNL Optimizely] 및 [!DNL Tealium]의 자사 세그먼트를 다음 유니버설 ID 파트너의 인증된(결정적) ID를 가진 세그먼트로 변환할 수 있습니다.
...
오스트레일리아 광고주는 [!DNL AdFixus]과(와) Advertising DSP 통합을 사용하여 [!DNL AdFixus] 유니버설 ID가 포함된 자사 세그먼트를 가져올 수 있습니다. 이 경로는 CDP 커넥터 내의 해시된 이메일 ID 또는 MAID를 [!DNL RampIDs] 또는 [!DNL UID2] ID로 변환하는 것과 별개입니다. 자세한 내용은 &quot;[자사 세그먼트 가져오기 [!DNL AdFixus]](/help/dsp/audiences/sources/source-adfixus.md)&quot;를 참조하십시오.
