...
[!UICONTROL Audience Analysis] 탭은 배치 수준에서 대상 세그먼트 타깃팅의 효과에 대한 실시간 통찰력을 제공합니다. 여기에는 시간 경과에 따른 세그먼트 크기 트렌드와 일별 입찰 funnel 분류가 포함됩니다. 이러한 통찰력을 사용하여 대상 대상 대상 풀의 안정성을 모니터링하고 대상 일치와 노출 게재 간에 볼륨이 손실되는 위치를 식별할 수 있습니다. 데이터는 대상 세그먼트를 타겟팅하는 배치에만 사용할 수 있습니다.

기본적으로 특정 광고주 및 특정 배치에 대한 데이터가 표시됩니다. 선택적으로 다른 광고주에 대한 데이터를 표시하도록 필터를 구성하거나 다른 배치를 선택할 수 있습니다.

인사이트는 다음과 같습니다.

* **[!UICONTROL Audience Segment Size Trends]:** 추세 차트는 배치에 대한 모든 대상 세그먼트의 일일 고유 사용자 수를 보여줍니다. 이 차트를 사용하여 시간이 지남에 따라 타겟 대상이 증가, 안정 또는 수축하고 있는지 모니터링합니다. 지속적인 감소는 세그먼트가 만료되거나 축소되고 있음을 나타낼 수 있으며 세그먼트 데이터를 새로 고치거나 타겟팅을 확장해야 할 수 있습니다.

  특정 데이터 포인트에 대한 정확한 사용자 수와 날짜를 보려면 해당 포인트 위에 커서를 놓습니다.
...
1. (선택 사항) 데이터 범위를 더 좁히려면 도구 모음에서 차원 유형을 선택한 다음 특정 차원(단일 캠페인, 패키지 또는 배치)을 선택합니다.

1. ([!UICONTROL Audience Funnel Analysis]만 해당, 선택 사항) 일별과 주별 사이의 시간 증분을 변경하려면 **[!UICONTROL Day]** 또는 **[!UICONTROL Week]**&#x200B;을(를) 선택합니다.
...
* 탭 오른쪽 상단에서 **[!UICONTROL ...]** > **[!UICONTROL Export]**&#x200B;을(를) 클릭합니다.
