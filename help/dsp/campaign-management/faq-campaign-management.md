...
* 배치나 패키지 설정을 변경할 때 변경 사항은 언제 적용됩니까?
  ...
  배송 마지막 날인 경우 당일 일찍 변경 작업을 수행하여 DSP에서 변경 사항을 기반으로 패키지를 재수정할 시간이 충분합니다. 예를 들어 짝수 게재 간격 방식에서 프론트로드 게재 간격 방식으로 변경하는 경우, DSP은 나머지 항공편 전체에 걸쳐 지출을 배분하는 방법을 재평가해야 합니다. 비행 마지막 날에 배달할 시간이 1시간밖에 남지 않았다면 그런 변화는 하지 마세요.

## 비행 중 예산 업데이트

* 배치를 변경하면 DSP에서 패키지 예산을 다시 할당하는 데 얼마나 걸립니까?
