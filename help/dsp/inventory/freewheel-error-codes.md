---
title: 오류 코드 [!DNL FreeWheel] 광고 제출
description: 광고 제출을 위해 반환되는 오류 코드를 참조하십시오. [!DNL FreeWheel].
...
# 오류 코드 [!DNL FreeWheel] 광고 제출

광고 제출 실패에 대한 오류 메시지는 Advertising DSP 또는 [!DNL FreeWheel]. 오류 메시지는 [!UICONTROL API Response] 열의 [[!UICONTROL Freewheel Status] 대화 상자](freewheel-check-status.md).

## Advertising DSP 내부 오류

| 오류 메시지 | 설명 | 다음 단계 |
|--- |--- |--- |
| [!DNL Awaiting status response from [!DNL FreeWheel]] | [!DNL FreeWheel] 제출 성공 또는 실패에 대해 아직 응답하지 않았습니다. | 10분 후에 상태를 다시 확인하십시오. |
| [!DNL The submitted ad does not have a clock number assigned.] | [!DNL FreeWheel] 에서는 지정된 시계 번호가 없는 영국 광고를 수락하지 않습니다. | 광고에 시계 번호를 할당한 다음 광고를 다시 제출합니다. |
...
| [!DNL The deal id you input is not setup as a guaranteed feed. Please submit guaranteed deals only.] | 제출된 거래는 프로그램 보증 거래로 설정되지 않습니다. [!DNL FreeWheel] 은 보장된 거래만 수락합니다. | 거래 ID를 프로그램 보증 거래로 설정합니다. 이 광고는에 자동으로 제출됩니다. [!DNL FreeWheel] 거래 ID 워크플로의 끝에 프로그래밍 방식으로 보장된 기본 배치를 저장하는 경우. |
| [!DNL Invalid external_deal_id:] \&lt;deal_id> | 제출된 거래 ID가 존재하지 않거나 Adobe 끝에서 활성화되지 않습니다. | 거래가 활성 상태인지 확인한 다음 광고를 다시 제출합니다. |
| [!DNL \[public_id=]\&lt;deal>] 없음 | 제출된 거래 ID가 [!DNL FreeWheel] 종료. | 다음으로 연락 [!DNL FreeWheel] 담당자가 거래 ID를 확인합니다. |
...
| 코드 | 의미 | 설명 | 다음 단계 |
|--- |--- |--- |--- |
| 401 | 승인되지 않음 | 액세스 자격 증명이 잘못되었거나 누락 또는 잘못되었습니다. | Adobe 계정 팀에 문의하십시오. |
