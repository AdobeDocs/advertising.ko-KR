...
1. 거래를 저장한 후 거래에 사용할 광고(또는 게시자 관리 광고의 1x1 추적 픽셀)를 선택하고 메시지가 표시되면 프로그램 보증 기본 배치(PG)를 만듭니다.
   ...
   구매의 100%를 게재하려면 거래에 대한 기본 PG 배치를 생성해야 합니다. 이 유형의 배치에는 타깃팅이 없으므로 DSP은 게시자의 모든 입찰 요청에 입찰을 반환할 수 있습니다.
...
1. 거래를 저장한 후 거래에 사용할 광고(또는 게시자 관리 광고의 경우 1x1 추적 픽셀)를 선택하고 메시지가 표시되면 PG 기본 배치를 만듭니다.
   ...
   구매의 100%를 게재하려면 거래에 대한 PG 기본 배치를 생성해야 합니다. 이 유형의 배치에는 타깃팅이 없으므로 DSP은 게시자의 모든 입찰 요청에 입찰을 반환할 수 있습니다.
