...
기존 레벨 사이에 대상 노드를 삽입하면 새 대상 노드는 기존 하위 대상 및 크리에이티브를 모두 유지하고 새 노드를 처음에는 &quot;모두&quot;라고 합니다. 더 많은 특정 대상을 추가하지 않고 선택적으로 새 노드를 유지할 수 있습니다.

특정 대상을 정의하려면 동일한 수준에서 형제 대상 노드를 추가하고 새 대상을 지정한 다음 해당 대상에만 크리에이티브를 할당합니다. 이렇게 하면 새 대상 노드가 생성되고 이전에 &quot;모두&quot;에 할당되었던 모든 하위 대상 및 크리에이티브가 동일한 레벨의 새 &quot;기타 모든&quot; 노드로 이동합니다. 이렇게 하면 새 동위 멤버 노드에만 새 타깃팅 정보가 포함되므로 기존 하위 분기는 새 대상의 추가에 의해 영향을 받지 않습니다.
...
1. 다음 중 하나를 수행합니다.
   ...
   * 형제 노드가 이미 있는 경우 다음을 수행합니다.
      ...
      * 재타겟팅 픽셀 타겟의 경우 사용할 단일 재타겟팅 픽셀과 광고 효과를 표시하기 위해 있어야 하는 픽셀 속성에 필요한 값을 선택한 다음 **[!UICONTROL Apply]**&#x200B;을(를) 클릭합니다.
...
1. 다음 중 하나를 수행합니다.
   ...
   * (선택 사항) [지정한 형식의 대상을 포함할 형제 대상 노드를 추가](experience-target-node-add-sibling.md)합니다.
