...
* *[!UICONTROL Immediately]*: 보고서를 보고서 큐에 즉시 추가합니다.
  ...
  >[!NOTE]
  >
  >[!UICONTROL Reports] 보기에서 [언제든지 사용자 지정 보고서를 실행](/help/dsp/reports/report-run-now.md)할 수도 있습니다.
...
* **\[규칙 유형\]:**(Adobe Advertising 전환 추적만 있는 광고주) 보고서 내에서 전환으로 이어지는 일련의 이벤트에서 전환 데이터를 특성화하는 방법. 규칙 간의 차이를 비교하려면 두 개 이상의 규칙을 선택할 수 있습니다.
...
| [!UICONTROL Dimension] | [!UICONTROL Ad] | [!UICONTROL Is Default] | 제공된 광고가 경험에 대한 기본 이미지 광고인지 여부. |
...
| [!UICONTROL Dimension] | [!UICONTROL DSP] | [!UICONTROL DSP ID] | 광고가 실행된 DSP ID입니다. |
