...
>[!PREREQUISITES]
>
>* 광고 템플릿: 디스플레이 광고 템플릿(HTML5 파일이 포함된 ZIP 파일) 또는 비디오 광고 템플릿(.scene 파일이 포함된 ZIP 파일)
>* CSV, TSV 또는 Microsoft Excel 스프레드시트(XLSX) 형식의 제품 카탈로그

1. Creative 라이브러리의 [동적 크리에이티브 만들기](/help/creative/creative-libraries/creative-add-dynamic.md). 다이내믹 HTML5 및 비디오 광고의 경우 기존 광고 템플릿과 카탈로그를 업로드하거나 선택하십시오.
...
1. 사용 가능한 자산을 기반으로 동적 광고의 [광고 템플릿을 만듭니다](/help/creative/ad-templates/ad-template-manage.md). 광고 템플릿은 ZIP 형식이어야 하며 다음 항목을 포함해야 합니다. <!-- Need to add more specs for templates -->

* 광고 크리에이티브 표시: 원하는 광고 형식을 갖는 HTML5 파일과 (동적 HTML5 광고만 해당) 광고 속성(.tdf)이 있는 파일

* 비디오 크리에이티브: 원하는 광고 포맷이 있는 .scene 파일과 광고 속성(.tdf)이 있는 파일
