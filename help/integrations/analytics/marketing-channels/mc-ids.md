...
[!DNL Marketing Channels] 채널에 대한 [!UICONTROL Paid Search] 처리 규칙은 다음과 같습니다.

![[!UICONTROL Paid Search] 규칙의 예](/help/integrations/assets/a4adc-mc-rule-paidsearch.png)

[!DNL Marketing Channels] 채널에 대한 [!UICONTROL YouTube Video Ads] 처리 규칙은 다음과 같습니다.
...
디스플레이 뷰스루 채널을 만들려면 EF ID가 &quot;:i&quot;(으)로 끝나는 규칙을 만듭니다. 방문자가 광고를 클릭하지 않았기 때문에 뷰스루 추적에는 URL에 `ef_id` 또는 `s_kwcid`이(가) 포함되지 않으므로 규칙에는 하나의 조건만 필요합니다.
