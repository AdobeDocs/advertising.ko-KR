---
title: Adobe 광고 ID 사용 [!DNL Analytics]
...
# Adobe 광고 ID 사용 [!DNL Analytics]

*Adobe Advertising-Adobe Analytics 통합 전용 광고주*

*Advertising DSP 및[!DNL Advertising Search, Social, & Commerce]*

Adobe 광고는 온사이트 성능 추적에 두 개의 ID를 사용합니다. a *EF ID* 그리고 *AMO ID*.

광고 노출이 발생하면 Adobe 광고에서 AMO ID 및 EF ID 값을 만들어 저장합니다. 광고를 본 방문자가 광고를 클릭하지 않고 사이트에 들어오면, [!DNL Analytics] 는 를 통해 Adobe 광고에서 이러한 값을 호출합니다 [!DNL Analytics for Advertising] JavaScript 코드. 뷰스루 트래픽의 경우, [!DNL Analytics] 는 보충 ID(`SDID`)을 클릭하여 EF ID와 AMO ID를 [!DNL Analytics]. 클릭스루 트래픽의 경우 이러한 ID는 `s_kwcid` 및 `ef_id` 쿼리 문자열 매개 변수.
...
* 사용자가 광고를 보고 사이트를 방문하지만 클릭하지 않은 경우 뷰스루 항목이 캡처됩니다. [!DNL Analytics] 두 가지 조건이 충족되는 경우 뷰스루를 기록합니다.
   * 방문자에 대한 클릭스루가 없습니다 [!DNL DSP] 또는 [!DNL Search, Social, & Commerce] 다음 기간 동안 광고 [전환 확인 기간 을 클릭합니다.](#lookback-a4adc).
...
* 사이트 방문자가 사이트로 들어오기 전에 광고를 클릭하면 클릭스루 항목이 캡처됩니다. [!DNL Analytics] 다음 조건 중 하나가 발생하면 클릭스루를 캡처합니다.
   * 이 URL에는 Adobe 광고에서 랜딩 페이지 URL에 추가한 EF ID와 AMO ID가 포함되어 있습니다.
   * URL에 추적 코드가 포함되어 있지 않지만, Adobe Advertising JavaScript 코드는 지난 2분 내에 클릭 수를 감지합니다.
...
EF ID는 Adobe 광고에서 온라인 클릭 또는 광고 노출과 활동을 연결하는 데 사용하는 고유한 토큰입니다. EF ID는 [!DNL Analytics] [eVar](https://experienceleague.adobe.com/docs/analytics/components/dimensions/evar.html) 또는 Var(예약된 eVar) 차원(Adobe 광고 EF ID)을 수행하고 개별 브라우저 또는 장치 수준에서 각 광고 클릭이나 노출을 추적합니다. EF ID는 주로 전송 키로 작동합니다 [!DNL Analytics] Adobe 광고 내에서 보고 및 입찰 최적화를 위해 Adobe 광고에 데이터를 보냅니다.

### EF ID 형식

>[!NOTE]
>
>EF ID는 대/소문자를 구분합니다. 다음과 같은 경우 [!DNL Analytics] 구현에서는 URL 추적을 소문자로 강제 적용하면 Adobe 광고에서 EF ID를 인식하지 못합니다. 이는 Adobe 광고 입찰 및 보고에 영향을 주지만 내에서 Adobe 광고 보고에 영향을 주지 않습니다 [!DNL Analytics].
...
* &lt;*Adobe 광고 방문자 ID*> 은 방문자당 고유한 ID(예: UhKVaAABCkJ0mDt)입니다. 또한 *서퍼 ID*.
...
AMO ID는 덜 세분화된 수준에서 각 고유 광고 조합을 추적하며 [!DNL Analytics] Adobe 광고에서 광고 지표(노출 횟수, 클릭 수 및 비용 등)에 대한 데이터 분류 및 섭취. AMO ID는에 저장됩니다. [!DNL Analytics] [eVar](https://experienceleague.adobe.com/docs/analytics/components/dimensions/evar.html) 또는 rVar 차원(AMO ID)이며,에서 보고에만 사용됩니다 [!DNL Analytics].
...
* &lt;*광고 ID*> 은 광고에 Adobe 광고 생성 고유 식별자를 사용합니다. Adobe 광고 엔티티 메타데이터를 읽을 수 있도록 변환하는 키 역할을 합니다 [!DNL Analytics] 차원.

* &lt;*배치 ID*>은(는) 배치에 대해 Adobe 광고에서 생성한 고유 식별자입니다. Adobe 광고 엔티티 메타데이터를 읽을 수 있도록 변환하는 키 역할을 합니다 [!DNL Analytics] 차원.

AMO ID 예: AC!iIMvXqlOa6Nia2lDvtgw!GrVv6o2oV2qQLjQiXLC7

### AMO ID 형식 [!DNL Search, Social, & Commerce]

용 AMO ID [!DNL Search, Social, & Commerce] 각 검색 엔진에 대해 고유한 형식을 따릅니다. 모든 검색 엔진의 형식은 다음과 같이 시작합니다.
...
* `AL` 은 광고 네트워크의 채널 ID입니다.
* `{userid}` 는 Adobe 광고주가 광고주에게 지정하는 고유 숫자 사용자 ID입니다.
* `{sid}` Adobe Advertising이 지정된 광고 네트워크에 사용하는 숫자 ID입니다(예: ) `3` 대상 [!DNL Google Ads] 또는 `10` 대상 [!DNL Microsoft Advertising].

다음은 두 개의 광고 네트워크에 대한 전체 AMO ID 형식입니다. 다른 광고 네트워크의 AMO ID 포맷은 Adobe 계정 팀에 문의하십시오.
...
내 모든 보고 또는 감사 [!DNL Analytics]를 설정하는 것이 좋습니다. 해당 인스턴스와 함께 AMO ID를 사용하는 것이 좋습니다. 자세한 내용은 &quot;[에 대한 데이터 유효성 검사 [!DNL Analytics for Advertising]](data-variances.md#data-validation)&quot;(다음 사이 의 예상되는 데이터 분산) [!DNL Analytics] 및 Adobe 광고.&quot;

## Analytics 분류 정보

in [!DNL Analytics], [분류](https://experienceleague.adobe.com/docs/analytics/components/classifications/c-classifications.html) 는 계정, 캠페인 또는 광고와 같은 주어진 추적 코드에 대한 메타데이터의 일부입니다. Adobe 광고는 보고서를 생성할 때 데이터를 다양한 방법(예: 광고 유형 또는 캠페인)으로 표시할 수 있도록 분류를 사용하여 원시 Adobe 광고 데이터를 분류합니다. 분류는 의 Adobe 광고 보고 기준을 형성합니다 [!DNL Analytics] 와 같은 AMO 지표와 함께 사용할 수 있습니다. [!UICONTROL AMO Cost], [!UICONTROL AMO Impressions], 및 [!UICONTROL AMO Clicks]와 같은 사용자 지정 및 표준 온사이트 이벤트 포함 [!UICONTROL Visits], [!UICONTROL Leads], [!UICONTROL Orders], 및 [!UICONTROL Revenue].
