---
title: ' [!DNL Last Event Service] 에서  [!DNL Web SDK]JavaScript 라이브러리 사용'
...
# Adobe Experience Platform [!DNL Last Event Service]에서 [!DNL Web SDK] JavaScript 라이브러리 사용
...
## 2단계: [!DNL Web SDK]을(를) 사용하여 [!DNL StitchID]에 대한 XDM 데이터로 [!DNL Analytics]을(를) 보냅니다.

[!DNL Web SDK] `sendEvent` 명령 내에 다음 속성을 삽입하여 [!DNL StitchID]에 대한 [!DNL Experience Edge]&#x200B;(XDM) 데이터로 [!DNL Experience Data Model]에 [!DNL Analytics]을(를) 보냅니다.<!-- The library sends the StitchID to [!DNL Experience Edge] as `[_adcloud.advertisingStitchID](https://github.com/adobe/xdm/blob/master/docs/reference/adobe/experience/adcloud/stitch.schema.md)`. --> [!DNL Analytics]이(가) 값을 `SDID`(으)로 사용합니다.
...
>* [에 대한  [!DNL Analytics for Advertising]](/help/integrations/analytics/javascript.md)JavaScript 코드
