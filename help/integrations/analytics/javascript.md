...
아래 설명된 대로 Adobe Advertising으로 이동하는 요청과 [!DNL Charles]&#x200B;(으)로 이동하는 요청 사이의 4가지 ID 값을 비교하여 패킷 스니퍼 유형의 도구([!DNL Fiddler], [!DNL Chrome Developer Tools] 또는 [!DNL Analytics])를 사용하여 유효성 검사를 수행할 수 있습니다.
...
1. [!UICONTROL Network] 탭을 `last`(으)로 필터링하고 다음 두 행을 검토하십시오.
   ...
   * 두 번째 행은 Adobe Advertising에 요청을 보내는 호출입니다. `_les_imsOrgId=[your_imsOrgId_here]&_les_url=[your_encoded_url]`(으)로 시작됩니다.
   ...
   1. 응용 프로그램 탭에서 `adcloud` 쿠키를 찾은 다음 값이 `_les_v`이고 30분 후에 만료되는 UTC 에포크 타임스탬프가 있는 `y`(마지막 방문)이 쿠키에 포함되어 있는지 확인합니다.
...
1. (Experience Cloud Identity 서비스 `visitorAPI.js` 코드를 사용하는 구현) Analytics 히트를 보려면 `/b/ss`에서 필터링합니다.
   ...
   ![에서 `/b/ss`](/help/integrations/assets/a4adc-code-validation-filter-bss.png)필터링
...
1. (Experience Platform [!DNL Web SDK] `alloy.js`코드를 사용하는 구현) `/interact`에서 필터링하여 Edge Network에 대한 요청 페이로드에 `advertisingStitchID`이(가) 포함되어 있는지 확인합니다.
   ...
   ![에서 `/interact`](/help/integrations/assets/a4adc-code-validation-filter-interact.png)필터링
...
1. 두 히트 간의 ID 값을 비교합니다. Analytics 히트의 보고서 세트 ID(`/b/ss/` 바로 뒤 URL 경로)를 제외한 모든 값은 쿼리 문자열 매개 변수에 있어야 합니다.
   ...
   | 결합 ID | stitchId | `advertisingStitchID` 속성 아래의 `_adcloud` |  |
   | Analytics 보고서 세트 | `/b/ss/` 이후의 값 | | `_les_rsid` |
   | Experience Cloud 방문자 ID | mid |  | `_les_mid` |

   ID 값이 일치하는 경우 JavaScript 구현이 확인됩니다. Adobe Advertising은 클릭스루 또는 뷰스루 추적 세부 정보(있는 경우)가 있는 경우 [!DNL Analytics] 서버에 보냅니다.
...
1. [!UICONTROL Request - Parameters] 행에서 생성된 신호를 감사합니다. &quot;[코드 확인 방법 [!DNL Chrome Developer Tools]](#validate-js-chrome)&quot;의 3단계와 유사합니다.
   ...
   * 코드가 생성되지 않으면 [!UICONTROL Application] 탭에서 Adobe Advertising 쿠키가 제거되었는지 확인하십시오. 페이지가 제거되면 페이지를 새로 고치고 프로세스를 반복합니다.
   ...
   ![에서 [!DNL Analytics for Advertising] JavaScript 코드 감사 [!DNL Experience Cloud Debugger]](/help/integrations/assets/a4adc-js-audit-debugger.png)
