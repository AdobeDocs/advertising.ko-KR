...
| 포함 코드가 누락되었거나 잘못된 환경 | 웹 페이지의 [[!DNL Tags] 포함 코드](https://experienceleague.adobe.com/en/docs/experience-platform/tags/publish/environments/environments)이(가) 올바른 환경(Dev/Stage/Prod)을 참조하는지 확인하십시오. `<head>` 태그에서 `//assets.adobedtm.com/...` 스크립트 태그에 대한 환경을 찾습니다. |
| 비동기 및 동기 로드 충돌 | 웹 페이지당 [[!DNL Tags] 포함 코드](https://experienceleague.adobe.com/en/docs/experience-platform/tags/publish/environments/environments)이(가) 하나만 있는지 확인하십시오. 포함 코드가 중복되면 경합 조건이 발생합니다. |
| CSP(콘텐츠 보안 정책) 차단 | [CSP `connect-src` 및 `script-src` 지시문](https://experienceleague.adobe.com/en/docs/experience-platform/collection/use-cases/configuring-a-csp)에 `edge.adobedc.net` `and assets.adobedtm.com`을(를) 추가합니다. |
...
* 규칙이 저장되고 활성 [라이브러리 빌드](https://experienceleague.adobe.com/en/docs/experience-platform/tags/publish/builds)에 포함됩니다.
* 이벤트 유형은 실제 페이지 동작과 일치합니다(예: [!UICONTROL Library Loaded] 대 [!UICONTROL DOM Ready] 대 [!UICONTROL Window Loaded]).
* 그 규칙의 조건은 그다지 제한적이지 않다. 문제를 격리하기 위해 일시적으로 [조건을 제거](https://experienceleague.adobe.com/en/docs/experience-platform/tags/ui/rules)하여 테스트하십시오.
* 규칙 순서가 올바릅니다. 여러 규칙이 동일한 이벤트를 공유하는 경우 [규칙 순서 지정](https://experienceleague.adobe.com/en/docs/experience-platform/tags/ui/rules)을 확인하십시오.
...
| 필수 Adobe Advertising 필드가 스키마에서 누락되었습니다. | 필수 Adobe Advertising 필드가 `_experience.adcloud.conversionDetails`의 [스키마](https://experienceleague.adobe.com/en/docs/platform-learn/implement-web-sdk/initial-configuration/configure-schemas)에 있는지 확인하십시오. &quot;[참조: 필수 스키마 필드](#required-schema-fields)&quot;을(를) 참조하십시오.<br><br>필드 중 하나가 누락된 경우 **Adobe Advertising Cloud ExperienceEvent 전체 확장** 필드 그룹이 스키마에 저장되었는지 확인한 다음 스키마 편집기를 새로 고치십시오. |
