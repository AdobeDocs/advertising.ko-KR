...
* 광고주는 [the [!DNL Adobe CX Enterprise Identity (ECID) Service]](https://experienceleague.adobe.com/en/docs/id-service/using/intro/overview) 버전 2.0 이상을 구현해야 합니다. [!DNL Identity Service]은(는) CX Enterprise의 모든 솔루션에서 방문자를 식별하는 범용 영구 ID를 제공합니다.

  구현에는 광고주 사이트의 각 웹 페이지에 [!DNL Identity service] 코드를 추가하는 작업이 포함됩니다.

* 조직은 CX Enterprise 서비스에 대해 [활성화됨](https://experienceleague.adobe.com/en/docs/core-services/interface/services/overview)이어야 하며 CX Enterprise [!DNL Organization ID]&#x200B;(이전 이름: [!DNL IMS org ID])이(가) 있어야 합니다.

  [!UICONTROL Organization ID]을(를) 사용하면 여러 Adobe CX Enterprise 제품이 있는 조직에서 일부 제품 간에 데이터를 공유할 수 있습니다.
...
* 두 대상을 모든 Audience Manager 세그먼트에 매핑하면 Audience Manager에서 Audience Manager에 사용된 동일한 CX Enterprise [!DNL Organization ID]과(와) 연결된 DSP 광고주 계정과 세그먼트를 공유할 수 있습니다.
...
1. [!DNL Search, Social, & Commerce] 내에서 [!DNL Search, Social, & Commerce] 구현 팀이나 직접 액세스 클라이언트 관리자 역할을 가진 사용자가 [!UICONTROL Search, Social, & Commerce] > [!UICONTROL Admin] > [!UICONTROL Audience Manager Setup]에서 가져오기를 시작해야 합니다.
   ...
   조직의 Adobe CX Enterprise [!DNL Organization ID]&#x200B;([!DNL IMS org ID])이(가) 필요합니다. ID는 조직의 Audience Manager 계정에 사용되는 ID와 동일해야 합니다.
