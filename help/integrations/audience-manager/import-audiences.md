...
DSP 또는 [!DNL Adobe]에서 [!DNL Creative]개의 대상에 액세스하려면 대상을 DSP으로 가져와야 합니다. [!DNL Adobe]에서 [!DNL Search, Social, & Commerce]명의 대상자에 액세스하려면 대상자를 [!DNL Search, Social, & Commerce]&#x200B;(으)로 가져와야 합니다.
...
* (광고주: [!DNL Analytics]) 광고주는 [&#x200B; [!DNL Analytics]  버전 1.6.4 이상을 사용하여 `appMeasurement.js`구현](https://experienceleague.adobe.com/ko/docs/analytics/implementation/js/overview)해야 합니다.
...
## Audience Manager 대상을 DSP으로 가져오기
...
### Audience Manager의 변경 사항은 무엇입니까?
...
* 두 대상을 모든 Audience Manager 세그먼트에 매핑하면 Audience Manager에서 Audience Manager에 사용된 동일한 Experience Cloud [!DNL Organization ID]과(와) 연결된 DSP 광고주 계정과 세그먼트를 공유할 수 있습니다.
...
1. [!DNL Search, Social, & Commerce] 내에서 [!DNL Search, Social, & Commerce] 구현 팀이나 직접 액세스 클라이언트 관리자 역할을 가진 사용자가 [!UICONTROL Search, Social, & Commerce] > [!UICONTROL Admin] > [!UICONTROL Audience Manager Setup]에서 가져오기를 시작해야 합니다.

   조직의 Experience Cloud [!DNL Organization ID]&#x200B;([!DNL IMS org ID])이(가) 필요합니다. ID는 조직의 Audience Manager 계정에 사용되는 ID와 동일해야 합니다.

### Audience Manager의 변경 사항은 무엇입니까?

Audience Manager에서 조직에서 두 개의 [!DNL Search, Social, & Commerce] 대상을 사용할 수 있게 되었습니다.
...
* (DSP 사용 광고주):
   ...
   * 세그먼트는 Adobe Advertising 디스플레이 광고에서 타겟팅됩니다.
...
DSP은 [!DNL Adobe Experience Cloud Identity (ECID) Service]을(를) 사용하여 데이터를 자동으로 동기화합니다. 동기화하는 동안 [!DNL ECID Service]이(가) [!DNL cm.everesttech.net]에 Adobe Advertising을 호출합니다. Adobe Advertising은 신뢰할 수 있는 도메인이므로 대부분의 타사 활성화 파트너와 마찬가지로 ID 동기화는 대상 게시 iframe 내에서가 아니라 상위 페이지에서 수행됩니다. Audience Manager은 [이라고도 하는  [!DNL Unique User ID (AAM UUID)]Audience Manager](https://experienceleague.adobe.com/ko/docs/audience-manager/user-guide/reference/ids-in-aam) [!DNL Device ID]을(를) 사용하여 장치 ID별로 고유 사용자를 식별합니다.
...
DSP은 Audience Manager 분류법에 따라 세그먼트 이름을 구성하고 해당 세그먼트 멤버십 수를 다음 위치에 포함합니다.

* [배치 설정](/help/dsp/campaign-management/placements/placement-settings.md#audience-targeting): [!UICONTROL Adobe Segments] 섹션의 [!UICONTROL Audience Targeting] 탭에서.
...
[!DNL Search, Social, & Commerce]에서 [!DNL Google] > [!UICONTROL Data Source] > [!UICONTROL Adobe Audience]에서 [!UICONTROL Campaigns] &quot;[!UICONTROL Audiences]&quot;을(를) 사용하여 [!UICONTROL Library] 대상자를 만들 때 세그먼트를 사용할 수 있습니다.
