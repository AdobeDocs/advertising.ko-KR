...
* 광고주는 [the [!DNL Adobe CX Enterprise Identity (ECID) Service]](https://experienceleague.adobe.com/ko/docs/id-service/using/intro/overview) 버전 2.0 이상을 구현해야 합니다. [!DNL Identity Service]은(는) CX Enterprise의 모든 솔루션에서 방문자를 식별하는 범용 영구 ID를 제공합니다.

  구현에는 광고주 사이트의 각 웹 페이지에 [!DNL Identity service] 코드를 추가하는 작업이 포함됩니다.

* 조직은 CX Enterprise 서비스에 대해 [활성화됨](https://experienceleague.adobe.com/ko/docs/core-services/interface/services/overview)이어야 하며 CX Enterprise [!DNL Organization ID]&#x200B;(이전 이름: [!DNL IMS org ID])이(가) 있어야 합니다.
