...
* Adobe Audience Manager 노출 데이터 및 클릭투피드를 사용하여 광고 메시징으로 현장 경험을 미러링하여 A/B 테스트를 설정하십시오. [!DNL Target] 대상.
...
1. DSP 노출 데이터에서 Audience Manager 세그먼트를 구성합니다.
   ...
   1. [규칙 기반 트레이트 만들기](https://experienceleague.adobe.com/docs/audience-manager/user-guide/features/traits/trait-builder/create-onboarded-rule-based-traits.html) Audience Manager에서 세그먼트를 만들 때 사용합니다.
      ...
      * 선택 `Ad Cloud` (으)로 **[!UICONTROL Data Source]**.

      * 트레이트 표현식의 경우 다음을 사용하십시오 `d_event` (으)로 **[!UICONTROL Key]** 및 `imp` (으)로 **[!UICONTROL Value]**.

   1. [테스트 세그먼트 설정](https://experienceleague.adobe.com/docs/audience-manager/user-guide/features/segments/segment-builder.html) Audience Manager의 새 트레이트에 대해 `Ad Cloud` (으)로 **[!UICONTROL Data Source]**.

      Audience Manager은 세그먼트를 표준 랜딩 페이지 경험을 받는 컨트롤 그룹과 개인화된 온사이트 경험을 받는 테스트 그룹으로 자동으로 분할합니다.

## 3단계: A/B 테스트 활동 설정 [!DNL Target] DSP용
...
1. [A/B 테스트 만들기](https://experienceleague.adobe.com/docs/target/using/activities/abtest/create/test-create-ab.html):
   ...
   1. 다음 범위 내 **[!UICONTROL Reporting Settings]**&#x200B;를 선택하고 **[!UICONTROL Company Name]** 및 **[!UICONTROL Report Suite]** DSP 계정에 연결되었습니다.
   ...
   1. 활동에 대상 추가:
      ...
      1. 선택 **[!UICONTROL Site Pages]** > **[!UICONTROL Landing Page]** > **[!UICONTROL Query]**&#x200B;을 누르고 다음에 DSP 배치 키를 입력합니다. **[!UICONTROL Value]** 클릭스루 대상에 대한 Target 쿼리 문자열 매개 변수를 사용할 필드입니다.
...
* 테스트가 실행된 Adobe Advertising 캠페인, 패키지 또는 배치와 관련된 작업 영역 내 패널을 만듭니다. 요약 시각화를 사용하여 Adobe Advertising 지표를 와 동일한 보고서에 표시 [!DNL Target] 성능을 테스트합니다.
...
* **[!UICONTROL Target Activities]**: A/B 테스트 이름

* **[!UICONTROL Target Experiences]**: 활동 내에 사용된 랜딩 페이지 경험의 이름

* **[!UICONTROL Target Activity]** > **[!UICONTROL Experience]**: 동일한 행의 활동 이름 및 경험 이름
...
>[!MORELIKETHIS]
>
>* [광고 검색, 소셜 및 상거래 광고를 위한 Adobe Target에서 A/B 테스트 구성](ab-tests-search.md)
